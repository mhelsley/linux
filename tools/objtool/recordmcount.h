			...
			unsigned int const symsec_sh_link)
{
	...
	uint_t const old_shstr_sh_size   = shstrsec->sh.sh_size;
	uint_t const old_shstr_sh_offset = shstrsec->sh.sh_offset;
	uint_t t = 1 + strlen(mc_name) + shstrsec->sh.sh_size;
	uint_t new_e_shoff;

	/*
	 * Update the ELF section table entry for the section name string
	 * table to point to the modified string table we're about to append.
	 */
	shstrsec->changed = true;
	shstrsec->sh.sh_size = t;
	shstrsec->sh.sh_offset = sb.st_size;
	...
	if (ulseek(fd_map, sb.st_size, SEEK_SET) < 0)
		return -1;
	if (uwrite(fd_map, old_shstr_sh_offset + (void *)ehdr, old_shstr_sh_size) < 0)
		return -1;
	if (uwrite(fd_map, mc_name, 1 + strlen(mc_name)) < 0)
		return -1;

	/* old(modified) Elf_Shdr table, word-byte aligned */
	if (ulseek(fd_map, t, SEEK_SET) < 0)
		return -1;
	t += sizeof(Elf_Shdr) * old_shnum;
	if (uwrite(fd_map, old_shoff + (void *)ehdr, sizeof(Elf_Shdr) * old_shnum) < 0)
		return -1;
	...
	if (uwrite(fd_map, &mcsec, sizeof(mcsec)) < 0)
		return -1;
	...
	if (uwrite(fd_map, mloc0, (void *)mlocp - (void *)mloc0) < 0)
		return -1;
	if (uwrite(fd_map, mrel0, (void *)mrelp - (void *)mrel0) < 0)
		return -1;

	ehdr->e_shoff = _w(new_e_shoff);
	ehdr->e_shnum = w2(2 + w2(ehdr->e_shnum));  /* {.rel,}__mcount_loc */
	if (ulseek(fd_map, 0, SEEK_SET) < 0)
		return -1;
	if (uwrite(fd_map, ehdr, sizeof(*ehdr)) < 0)
		return -1;
	return elf_write(lf);
}
		      ...
		      const char *const txtname)
{
	...
	for (t = nrel; t; --t) {
		...
		if (!ret) {
			...
			if (ulseek(fd_map, (void *)relp - (void *)ehdr, SEEK_SET) < 0)
				return -1;
			if (uwrite(fd_map, &rel, sizeof(rel)) < 0)
				return -1;
		}
		relp = (Elf_Rel const *)(rel_entsize + (void *)relp);
	}
...
do_func(Elf_Ehdr *const ehdr, char const *const fname, unsigned const reltype)
{
	...
	if (!result && mloc0 != mlocp) {
		result = append_func(ehdr, shstrsec,
